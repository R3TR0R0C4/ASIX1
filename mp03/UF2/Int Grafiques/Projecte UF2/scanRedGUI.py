...
if "not found" in tracerouteDetect.stderr:#Si no trobem el paquet:
    ...
    #ethtools per veure el link-speed, duplex-speed i link-status
    ...
    #traceroute a la gateway (variable pasarel·la agarrada de la funcio ipRoute)
    ...
    #funcio per mostrar les normes de iptables
    def iptables():
        try:
    ...
    def finestraSpeedtest():
        ...
        testSpeed.title("Test de velocitat")
        testSpeed.resizable(False, False)
        
        botoCorreTest = Button(testSpeed, text="Speedtest", command=ferSpeedtest)
        botoCorreTest.pack()

        global labelDownload, labelUpload, labelPing, labelTimestamp, labelServidor, labelCiutat #agarrem les variables de l'anterior funcio

        #configurem valors de labels:
    ...
    #boto per realitzar el traceroute extern
    ...
    #boto per realitzar el traceroute intern
    ...
    #label per mostrar els resultats del nslookup
    labelNslookup=Label()
    labelNslookup.pack()
    ...
    #boto per cridar la finestra amb iptables
    botoIPtables=Button(
    ...
    #boto per cridar la finestra del test de velocitat
